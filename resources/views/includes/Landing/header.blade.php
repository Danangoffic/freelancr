        ...
        <div class=" mx-auto flex flex-wrap flex-row items-center justify-between">
            ...
            <div class="hidden lg:flex lg:items-center lg:w-auto w-full lg:ml-auto lg:mr-auto flex-wrap items-center text-base justify-center"
                ...
                <nav
                    ...
                    <a href="{{ route('index') }}"
                        class="block nav-link {{ \Route::is('index') ? 'active font-medium' : 'text-serv-text' }} ">Home</a>
                    <a href="{{ route('landing.explorer') }}"
                        class="block nav-link {{ \Route::is('landing.explorer') || \Route::is('landing.detail') ? 'active font-medium' : 'text-serv-text' }} ">Explore</a>
                    ...
                    @auth
                        ...
                        <form action="href="{{ route('logout') }}"" method="post">
                            @csrf
                            <div>
                                <button type="submit" class="block lg:hidden nav-link text-serv-text">Logout</button>
                            </div>
                        </form>
                    @endauth
                </nav>
                ...
                <div @click.away="open = false" class="hidden lg:block relative" x-data="{ open: false }">
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-100"
                        ...
                        <div class="px-2 py-2 bg-white rounded-md shadow dark-mode:bg-gray-800">
                            ...
                            <form action="{{ route('logout') }}" method="post" id="logout-landing-desktop">
                                @csrf
                                <div>
                                    <a href="{{route('logout')}}" onclick="lo('#logout-landing-desktop')"
                                        class="block text-left px-4 py-2 mt-2 text-sm bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:hover:bg-gray-600 dark-mode:focus:bg-gray-600 dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:text-gray-200 md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline">
                                        Logout
                                    </a>
                                </div>
                            </form>
